...
export function リスト表示(属性: any): void {
  重複と空エントリを削除(属性.リスト);
  const フォーム = document.querySelector("#" + 属性.フォームid);
  if (!フォーム) return;

  while (フォーム.firstChild) {
    フォーム.removeChild(フォーム.firstChild);
  }
  ...
  for (i = 0; i < 属性.リスト.length; i++) {
    const エントリ = document.createElement("span");

    const input = document.createElement("input");
    input.type = "text";
    input.className = 属性.クラス;
    input.id = `name${i}`;
    input.value = 属性.リスト[i];
    input.addEventListener("change", n個目の答を登録する関数(i, 属性));
    エントリ.appendChild(input);

    const checkSpan = document.createElement("span");
    checkSpan.textContent = "✖";
    checkSpan.className = "check";
    checkSpan.addEventListener("click", n個目の答を削除する関数(i, 属性));
    エントリ.appendChild(checkSpan);

    フォーム.appendChild(エントリ);

    if (属性.改行あり) {
      フォーム.appendChild(document.createElement("br"));
    }
  }
...
export function ランダムに回答を追加(リスト: string[]): void {
  ...
  const 新たに登録するデータ =
    未登録のリスト[Math.floor(Math.random() * 未登録のデータの数)];
...
export function ランダムに問題を追加(): void {
  const 未登録のリスト = db.問題例リスト.filter((item: string) =>
    !questions.includes(item)
  );
  const 未登録のデータの数 = 未登録のリスト.length;
  if (未登録のデータの数 == 0) return;
  const 新たに登録するデータ =
