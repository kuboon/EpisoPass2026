...
import { answer, editor } from "./editor.ts";
...
const main = async function (): Promise<void> {
  ...
  // ボタンの挙動設定
  document.querySelector("#descbutton")?.addEventListener("click", () => {
    ...
    (document.querySelector("#editbuttondiv") as HTMLElement).style.background =
      "#555";
    (document.querySelector("#dasbuttondiv") as HTMLElement).style.background =
      "#555";
    (document.querySelector("#dasbutton") as HTMLElement).style.display =
      "none";
  });
  document.querySelector("#episodbbutton")?.addEventListener("click", () => {
    episodb();
  });
  ...
  // 何度も登録されて困った
  const dasButton = document.querySelector("#dasbutton");
  if (dasButton) {
    const newDasButton = dasButton.cloneNode(true);
    dasButton.parentNode?.replaceChild(newDasButton, dasButton);
    newDasButton.addEventListener(
      "click",
